...
SUMMARY_TSV="${ROOT_DIR}/results/benchmarks/stage7_run_summary.tsv"
DATASET_ID="${DATASET_ID:-GSE267401}"
DATASET_ROOT="${DATASET_ROOT:-data/raw/${DATASET_ID}/extracted}"
SAMPLE_ID="${SAMPLE_ID:-GSM8265211_CTC21P}"
K_GRID="${K_GRID:-4,6}"
SEED="${SEED:-11}"
BAYES_INSTALL="${BAYES_INSTALL:-0}"
NOTE_PREFIX="${NOTE_PREFIX:-stage7-bayesspace-crosscohort}"
...
if ! "${R_CHECK_CMD[@]}" >"${tmp_log}" 2>&1; then
  ...
  if ! Rscript scripts/run_bayesspace_baseline.R \
    ...
    tail -n +2 "${tmp_tsv}" | sed 's/^/Fig2D\t/' >> "${ROOT_DIR}/results/figures/fig2_benchmark_summary.tsv"
    while IFS=$'\t' read -r ds sid mid mf prep pset kval seed_cnt s_ari s_iqr sp sp_iqr mk mk_iqr wt mem fail notes; do
      printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
        "Fig4D" "${ds}" "${sid}" "${mid}" "${kval}" "success" "${wt}" "${mem}" "1" "0" "local-first" "${notes}" \
...
append_stage7_summary "stage7-bayesspace-crosscohort" "${started_utc}" "${finished_utc}" "${status}" "${before_count}" "${after_count}" "${added_count}" "${NOTE_PREFIX}"

if [[ "${status}" != "success" || "${added_count}" -le 0 ]]; then
  echo "Stage-7 finished without BayesSpace additions. Check stage7_run_summary.tsv and failure_log.tsv"
  exit 1
fi

echo "Stage-7 completed with BayesSpace rows added: ${added_count}"
