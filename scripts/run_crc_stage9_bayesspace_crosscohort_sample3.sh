...
SUMMARY_TSV="${ROOT_DIR}/results/benchmarks/stage9_run_summary.tsv"
DATASET_ID="${DATASET_ID:-GSE267401}"
DATASET_ROOT="${DATASET_ROOT:-data/raw/${DATASET_ID}/extracted}"
SAMPLE_ID="${SAMPLE_ID:-GSM8265213_CTC17P}"
SEEDS="${SEEDS:-23,37}"
K_GRID="${K_GRID:-4,6}"
BAYES_INSTALL="${BAYES_INSTALL:-0}"
NOTE_PREFIX="${NOTE_PREFIX:-stage9-bayesspace-crosscohort-sample3}"
...
for seed in "${seed_list[@]}"; do
  ...
  if ! Rscript scripts/run_bayesspace_baseline.R \
    ...
    tail -n +2 "${tmp_tsv}" | sed 's/^/Fig2F\t/' >> "${ROOT_DIR}/results/figures/fig2_benchmark_summary.tsv"
    while IFS=$'\t' read -r ds sid mid mf prep pset kval seed_cnt s_ari s_iqr sp sp_iqr mk mk_iqr wt mem fail notes; do
      printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
        "Fig4F" "${ds}" "${sid}" "${mid}" "${kval}" "success" "${wt}" "${mem}" "1" "0" "local-first" "${notes}" \
  ...
  append_stage9_summary "stage9-bayesspace-crosscohort-sample3" "${DATASET_ID}" "${SAMPLE_ID}" "${seed}" "${started_utc}" "${finished_utc}" "${status}" "${before_count}" "${after_count}" "${added_count}" "${note}"
done
...
if [[ "${overall_status}" != "success" || "${added_total}" -le 0 ]]; then
  echo "Stage-9 finished without full cross-cohort sample-3 success. Check stage9_run_summary.tsv and failure_log.tsv"
  exit 1
fi

echo "Stage-9 completed with BayesSpace rows added: ${added_total}"
